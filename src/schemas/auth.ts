...
export const registerSchema = yup
  .object({
    full_name: yup.string().trim().min(2, 'Full name is required.').required('Full name is required.'),
    display_name: yup.string().trim().max(60, 'Display name is too long.').notRequired(),
...
export type BasicRegistrationSchema = yup.InferType<typeof basicRegistrationSchema>;
...
export const twoFactorChallengeSchema = yup
  .object({
    otp: yup.string().trim().matches(/^[0-9]{6}$/, 'OTP must be 6 digits.').required('OTP is required.'),
    challenge_token: yup.string().trim().notRequired(),
  })
  .strict();

export type TwoFactorChallengeSchema = yup.InferType<typeof twoFactorChallengeSchema>;
...
export const phoneVerifySchema = yup
  .object({
    code: yup.string().trim().matches(/^[0-9]{4,8}$/, 'Enter the verification code.').required('Code is required.'),
  })
  .strict();
